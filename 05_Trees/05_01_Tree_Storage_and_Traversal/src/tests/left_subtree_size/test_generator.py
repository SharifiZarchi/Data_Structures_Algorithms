...
node_degree[node_list[0]] += 1

for i in range(n - 1):
    ind = randint(0, i)
    t.add_new_node_by_label(node_list[ind] + 1, node_list[i + 1] + 1)
target_node = node_list[randint(0, n - 1)] + 1
...
with open("input/input" + str(file_name) + ".txt", "w") as input_file:
	...
	input_file.write(str(target_node) + "\n")
with open("output/output" + str(file_name) + ".txt", "w") as output_file:
    output_file.write(str(t.get_subtree_size(t.find_by_label(target_node).left_child)) + "\n")
